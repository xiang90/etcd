...
func runLeaseRenewer(eps []string) {
	...
	for {
		...
		for {
			log.Printf("about to renew lease")
			lk, err = c.Lease.KeepAliveOnce(ctx, l.ID)
			if grpc.Code(err) == codes.NotFound {
				if time.Now().Sub(expire) < 0 {
					log.Printf("bad renew! exceeded: %v", time.Now().Sub(expire))
					for {
						lk, err = c.Lease.KeepAliveOnce(ctx, l.ID)
						fmt.Println(lk, err)
...
func runRacer(eps []string, round int) {
	...
	for i := range racers {
		racers[i] = concurrency.NewMutex(randClient(eps), prefix)
		wg.Add(1)
